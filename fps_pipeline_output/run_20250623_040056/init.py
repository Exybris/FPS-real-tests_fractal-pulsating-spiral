...
def init_strates(config):
    ...
    spiral_mode = str(coupling_cfg.get("type", "")).lower() == "spiral"
    W_spiral = None
    if spiral_mode:
        N_total = len(config["strates"])
        c_val = coupling_cfg.get("c", 0.25)
        closed_val = coupling_cfg.get("closed", False)
        W_spiral = generate_spiral_weights(N_total, c=c_val, closed=closed_val)
        print(f"🔄 Génération matrice de poids spiralés (c={c_val}, closed={closed_val})")
