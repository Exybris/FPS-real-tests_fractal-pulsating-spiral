*Lire dans d’autres langues : [🇫🇷 Français](README.fr.md), [🇬🇧 English](README.md).*
...
1. Initialisation de l’environnement, strates et dossiers de sortie
1. Simulation FPS (run principal avec `simulate.py`) :
...
1. Simulation Kuramoto (contrôle oscillateurs classiques)
1. Simulation Neutral (contrôle sans rétroaction)
1. Exploration des émergences via `explore.py`
1. Analyse batch (5 runs FPS pour validation statistique) via `analyze.py`
1. Génération des visualisations via `visualize.py`
1. Rapport final HTML avec comparaison des trois modes
...
1. **Prior prospectif** : `En(t) = (1-λ) · En(t-dt) + λ · κ · On(t-T)` — trace lissée et différée
1. **Scores sur O(t)** : 7 scores normalisés (stabilité, régulation, fluidité, résilience, innovation, effort, CPU)
1. **Sélection du prior perceptif S(t)** : prior correspondant au score le plus faible, ou mode neutre S(t)=O(t) si tous satisfaisants
1. **Latence** : `γ(t)` via `adaptive_aware` — optimise les scores calculés sur S(t)
1. **Régulation** : `G(x)` adaptatif — transforme l’erreur (E-O) en correction bornée, archétype choisi selon contexte
1. **Feedback** : `Fn_A(t) = βn · G_value` (→ amplitude) et `Fn_f(t) = βn · γ` (→ fréquence)
1. **Mise à jour A₀** : `A₀ ← A₀(1-ρ) + An(t)·ρ`, plancher min_amplitude (mémoire lente d’énergie)
1. **38 métriques** calculées et loguées
